...
/**
 * DecoratorViewPager父控件FrameLayout。
...
public class DecoratorLayout<T> extends FrameLayout {
    ...
    private DecoratorViewPager decoratorViewPager;// ViewPager轮播栏
    private DecoratorAdapter<T> decoratorAdapter;// ViewPager适配器
    ...
    public DecoratorLayout(Context context, @Nullable AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public DecoratorLayout(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
    ...
    private void init() {
        ...
        // 构造ViewPager
        decoratorViewPager = new DecoratorViewPager(mContext);
        // 设置ViewPager页面切换监听事件
        decoratorViewPager.addOnPageChangeListener(new ViewPageChangeListener());
        this.addView(decoratorViewPager);
     ...
     * @param datas         数据源
     */
    public DecoratorLayout init(HolderCreator holderCreator, List<T> datas) {
        this.mDatas = datas;
        // 设置Adapter
        ...
        realCount = decoratorAdapter.getRealCount();
        // 从中间开始显示
        what.set(totalCount / 2);
        decoratorViewPager.setCurrentItem(totalCount / 2);
        return this;
    }
     ...
     */
    public DecoratorLayout setPageIndicator(int selectedRes, int defalutRes) {
     ...
     * @param duration 停顿时间
     */
    public DecoratorLayout setDuration(long duration) {
     ...
     */
    public DecoratorLayout setPageIndicatorAlign(PageIndicatorAlign align) {
     ...
     */
    public DecoratorLayout setPointViewVisible(boolean isVisible) {
     ...
     * 设置ViewPager是否能够手动滚动
     *
     * @param canScroll true 可以  false 不可以 默认true
     */
    public DecoratorLayout setCanScroll(boolean canScroll) {
        if (decoratorViewPager != null)
            decoratorViewPager.setCanScroll(canScroll);
        return this;
    }
    ...
    private static class IVPagerHandler extends Handler {
        ...
        IVPagerHandler(DecoratorLayout layout) {
        ...
        public void handleMessage(Message msg) {
            ...
            DecoratorLayout current = mDecoratorLayout.get();
            try {
                // 实现ViewPager跳转
                int index = msg.what;
                int position;
                ...
                current.decoratorViewPager.setCurrentItem(position);
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
